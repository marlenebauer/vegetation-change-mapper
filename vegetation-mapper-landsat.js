//#############################################################################################################
//#                  LANDTRENDR CHANGE MAPPER — Landsat 4–9 adaptation (1990–Present)                         #
//#-----------------------------------------------------------------------------------------------------------#
...
//#   - Built a combined Landsat 4/5/7/8/9 Collection-2 L2 input with per-sensor band harmonisation.          #
//#   - Added scale-factor correction and QA_PIXEL-based cloud/shadow masking.                                #
...
exports.getIndexSelect = function(indexPanel) {
  // indexPanel.widgets().get(1) gets the ui.Select widget
  // .getValue() gets its current value
...
//  functions to select and rename bands for Landsat-8/9
...
//Create Landsat image collectio
var getSRcollection = function(year, startDay, endDay, sensor, aoi) {
  ...
  // Now apply processing steps
  collection = collection
    .map(cloudMask)  // Apply cloud mask
    .map(applyScaleFactors)  // Apply scale factors
    .map(function(image) {
      var renamed = ee.Algorithms.If( // rename bands
        sensor === 'LC08' || sensor === 'LC09',
        renameBandsOLI(image),
        renameBandsTM_ETM(image)
      );
      return ee.Image(renamed)
...
// Function to merge collections from different Landsat sensors (Landsat 4- 9)
